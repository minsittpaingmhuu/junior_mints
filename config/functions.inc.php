...
function emptyInput($name, $phone, $email, $type, $payment, $ticket_number){
    $result;
    if(empty($name) || empty($phone) || empty($email) || empty($type) || empty($payment)|| empty($ticket_number)){
...
function makeOrder($conn, $name, $phone, $email, $type, $payment, $ticket_number, $remarks){
    $sql = "INSERT INTO ticket_order (name, phone, email, type, payment, ticket_number, remarks) VALUES (?,?,?,?,?,?,?);";
    $stmt = mysqli_stmt_init($conn);
    if (!mysqli_stmt_prepare($stmt, $sql)){
        header("location: /getticket?error=orderfailed");
        exit();
    }
    mysqli_stmt_bind_param($stmt, "sssssss", $name, $phone, $email, $type, $payment, $ticket_number, $remarks);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
